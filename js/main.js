...
function renderCinema() {
    ...
    for (var i = 0; i < gCinema.length; i++) {
        ...
        for (var j = 0; j < gCinema[0].length; j++) {
            ...
            strHTML += `\t<td title="${title}"  data-i="${i}" data-j="${j}" 
    ...
    for (var i = rowIdx - 1; i <= rowIdx + 1; i++) {
        ...
        for (var j = colIdx - 1; j <= colIdx + 1; j++) {
            ...
            if (currCell.isSeat && !currCell.isBooked){
                const neighbor = document.querySelector(`[data-i="${i}"][data-j="${j}"]`);
                neighbor.classList.add("highlight");
                setTimeout(removeHighlight, 2500, neighbor);
            };
        };
    };;
};

function removeHighlight(neighbor) {
    neighbor.classList.remove("highlight");
};

function showNeighbor() {
    highlightAvailableSeatsAround(gCinema, +gElSelectedSeat.dataset.i, +gElSelectedSeat.dataset.j);
};